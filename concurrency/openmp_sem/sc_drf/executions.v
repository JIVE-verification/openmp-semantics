...
Module Executions.
  ...
  Section Executions.
    ...
    (** Connection between [multi_step] and [coarse_execution]*)
    Lemma coarse_execution_multi_step:
      forall U  tr tr' tp tre m tp' tre' m',
        coarse_execution (U, tr, tp, tre) m ([::], tr', tp', tre') m' ->
        multi_step (U, tr, tp, tre) m ([::], tr', tp', tre') m'.
    Proof.
      intros.
      induction H;
        econstructor;
    ...
    Lemma coarse_execution_schedule:
      forall U U' tr tr' tp tre m tp' tre' m',
    ...
    Proof.
      intros.
      inv Hstep; simpl in *;
        try (apply app_eq_refl_nil in H7; subst);
        simpl in H, H0;
        try (by exfalso);
    ...
    (* Lemma multi_step_inv_ext:
      ...
      exists U'' U''' tp'' tre'' m'' tr'' tp''' tre''' m''',
        multi_step (U, tr, tp, tre) m (U'', tr ++ tr'', tp'', tre'') m'' /\
        MachStep (U'', tr ++ tr'', tp'', tre'') m''
                 (U''', tr ++ tr'' ++ [:: ev], tp''', tre''') m''' /\
        multi_step (U''', tr ++ tr'' ++ [:: ev], tp''', tre''') m'''
                   (U', tr ++ tr', tp', tre') m' /\
        length tr'' = i.
    Proof.
      intros.
    ...
    Lemma multi_step_inv:
      forall U U' i tr tr' tp tre m tp' tre' m' ev
        (Hi: nth_error tr' i = Some ev)
        (Hexec: multi_step (U, tr, tp, tre) m (U', tr ++ tr', tp', tre') m'),
      exists U'' U''' tp'' tre'' m'' tr'' pre_ev post_ev  tp''' tre''' m''',
        multi_step (U, tr, tp, tre) m (U'', tr ++ tr'', tp'', tre'') m'' /\
        MachStep (U'', tr ++ tr'', tp'', tre'') m''
                 (U''', tr ++ tr'' ++ pre_ev ++ [:: ev] ++ post_ev, tp''', tre''') m''' /\
        multi_step (U''', tr ++ tr'' ++ pre_ev ++ [:: ev] ++ post_ev , tp''', tre''') m'''
                   (U', tr ++ tr', tp', tre') m' /\
        length (tr'' ++ pre_ev) = i.
    Proof.
      (* intros U.
      ...
      - inversion Hexec.
        ...
        + subst.
          ...
          * erewrite nth_error_app1 in Hi by ssromega.
            ...
            exists (a :: U), U, tp, tre, m, [::], l1, l2, tp'0, tre'0, m'0.
            subst.
            ...
            split; simpl; eauto.
          * erewrite nth_error_app2 in Hi by ssromega.
            specialize (IHU U' _ (tr ++ tr'0) tr'' tp'0 tre'0 m'0 tp' tre' m' _ Hi).
            rewrite <- app_assoc in IHU.
            specialize (IHU H11).
            destruct IHU as (U'' & U''' & tp'' & tre'' & m'' & tr''0 & pre_ev & post_ev
                             & tp''' & tre''' & m''' & Hexec0' & Hstep & Hexec''' & Hnth).
            exists U'', U''', tp'', tre'', m'', (tr'0 ++ tr''0), pre_ev, post_ev, tp''', tre''', m'''.
            split.
...
*)
  ...
  Section CoarseGrainedExecutions.
    Context {ge:Clight.genv}.
    ...
    Lemma step_event_sched:
      forall U tr tp tre m U' ev tr_pre tr_post tp' tre' m'
        (Hstep: MachStep (U, tr, tp, tre) m
...
(*       
    ...
    Lemma step_ev_contains:
      forall U tr tp tre m U' ev
        tr_pre tr_post tp' tre' m'
        (Hstep: MachStep (U, tr, tp, tre) m
                         (U', tr ++ tr_pre ++ [:: ev] ++ tr_post, tp', tre') m'),
    ...
    Proof.
      ...
      right; auto.
      eapply step_mem_compatible in H9.
      left; auto.
    Qed.
    ...
    Proof.
      ...
      inversion Hexec; subst.
      right; auto.
      eapply step_invariant in H9.
      left; auto.
    Qed.

    Lemma multi_step_containsThread :
      forall U tp tr tre m U' tp' tr' tre' m' i
        (Hexec: multi_step (U, tr, tp, tre) m (U', tr', tp', tre') m'),
  ...
  empty in all its resources) *)
    ...
    Lemma step_deadLocation:
      forall U U' tr tp tre m tr' tp' tre' m' b ofs
      ...
      - inversion Hdead.
        ...
        + intros.
          ...
          * subst.
            ...
            rewrite Hthreads.1 Hresources.1 in Hangel1.
            rewrite Hthreads.2 Hresources.2 in Hangel2.
      ...
      - inversion Hdead.
        ...
        + intros.
          ...
          * subst.
            ...
            specialize (Hthreads _ Htid).
            specialize (Hresources _ _ HisLock).
            rewrite Hthreads.1 in Hangel1.
            rewrite Hthreads.2 in Hangel2.
            rewrite gLockSetRes gssThreadRes.
            simpl.
  ...
  will be in the [lockRes] and thread i will have lock permission on addr*)
    Lemma Mklock_lockRes:
      forall i U tr tp tre m tp' tre' m' addr
        (Hstep: MachStep (i :: U, tr, tp, tre) m
                         (U, tr ++ [:: external i (Events.mklock addr)], tp', tre') m'),
        lockRes tp' addr /\
    ...
    Proof.
      intros.
      inv Hstep; simpl in *;
        try (apply app_eq_refl_nil in H5; discriminate).
      apply app_inv_head in H6.
      ...
      inv Htstep.
      rewrite FinPool.gsslockResUpdLock; split; auto.
      intros cnti'.
      rewrite FinPool.gLockSetRes FinPool.gssThreadRes.
      rewrite <- Hlock_perm.
      intros.
    ...
    Lemma remLockRes_Freelock:
      forall i U tr tr' tp tre m tp' tre' m' addr
    ...
    Proof.
      ...
      inv Hstep; simpl in *;
        try (inversion Htstep;
             subst tp');
        try (rewrite FinPool.gsoThreadCLPool; auto);
        try (rewrite FinPool.gsoThreadLPool; auto);
        try subst tp'0; try subst tp''; auto.
      - (** initial core case *)
        split; auto.
        unfold isLock in *.
        inv HschedN.
      ...
      - unfold isLock in *.
        ...
        rewrite FinPool.gssLockRes; auto.
        rewrite FinPool.gsoLockRes; eauto.
        intros ofs0 Hintv.
      ...
      (* - unfold isLock in *.
        inv HschedN.
        split;
          first by (rewrite FinPool.gsoAddLPool FinPool.gsoThreadLPool;
                    assumption).
        intros ofs0 Hintv.
        specialize (Hangel2 (addr.1) ofs0).
        apply permjoin_readable_iff in Hangel2.
      ...
      - (** Makelock case*)
        ...
        + assert ((b, Ptrofs.intval ofs) <> laddr').
          { intros Hcontra; subst. rewrite Hres in HlockRes. congruence. }
          right.
          exists laddr', rmap'.
          erewrite gsoLockRes by eauto.
          rewrite gsoThreadLPool.
      ...
      - (** Interesting case: freelock *)
        ...
        rewrite FinPool.gsolockResRemLock; [|intros Hcontra; subst; auto].
              ...
              { destruct (zle (ofs' + lksize.LKSIZE)%Z (Ptrofs.intval ofs)).
                ...
                - destruct (zlt ofs' (Ptrofs.intval ofs + lksize.LKSIZE)%Z); eauto.
                  ...
                  + exfalso.
                    ...
                    destruct (lockRes(ThreadPool := FinPool.FinThreadPool) tp (b', ofs')) eqn:Hlock'.
                    * rewrite Hlock' in Hvalid.
                      specialize (Hvalid (Ptrofs.intval ofs)).
                      setoid_rewrite Hvalid in His_lock.
                      congruence.
                      lia.
                    * rewrite /lockRes /= in Hlock'.
                      admit.
                  + exfalso.
  ...
  state *)
    Lemma remLockRes_Freelock_execution:
      forall U U' tr tr' tp tre m tp' tre' m' addr
        (Hlock: lockRes tp addr)
        (HisLock: isLock tp addr)
        (Hstep: multi_step (U, tr, tp, tre) m
                           (U', tr ++ tr', tp', tre') m')
              ...
              - intros n0 Hin.
                ...
                + inv Heq.
                  ...
                  * rewrite Hlength in Hintv.
                    erewrite Z2Nat.id in Hintv by lia.
      ...
      - simpl in Helim.
        ...
        + destruct Helim as [m'' [Halloc Helim']].
          ...
          { destruct (zle lo ofs').
            ...
            - assert (Hfreeable:permission_at m'' b0 ofs' Cur = None).
                by (eapply MemoryLemmas.permission_at_alloc_3; eauto; left; lia).
                pose proof (Mem.valid_new_block _ _ _ _ _ Halloc) as Hvalid.
                pose proof (ev_elim_stable _ ofs' _ Hvalid ltac:(rewrite Hfreeable; simpl; now constructor) Helim') as Heq.
                rewrite <- Heq.
                rewrite Hfreeable.
...
- Releasing a lock *)
              ...
              - constructor.
                ...
                + (** no thread has permission on [(b, ofs)]*)
                  ...
                  * rewrite gssThreadRes.
                    ...
                    simpl in Hcoh.
                    destruct ((getThreadR cnt).2 !!!! b ofs) eqn:?.
                    -- rewrite Heqo // in Hcoh.
                    -- done.
              ...
              - constructor.
                ...
                + intros.
                  ...
                  * pf_cleanup.
                    ...
                    eapply ThreadPoolWF.mem_compatible_invalid_block with (ofs := ofs) in Hinvalid; last apply Hcmpt.
                  ...
                  * (** case i is a different thread than the one that stepped*)
                    ...
                    eapply ThreadPoolWF.mem_compatible_invalid_block with (ofs := ofs) in Hinvalid; last apply Hcmpt.
                    rewrite! gsoThreadRes; auto.
                    rewrite (Hinvalid.1 _ cnti).2 (Hinvalid.1 _ cnti).1. done.
                + intros.
                  rewrite gsoThreadLPool in H.
                  erewrite restrPermMap_valid in Hinvalid.
                  eapply ThreadPoolWF.mem_compatible_invalid_block with (ofs := ofs) in Hinvalid; last apply Hcmpt.
                  rewrite (Hinvalid.2 _ _ H).2 (Hinvalid.2 _ _ H).1.
                  done.
            }
          }
      ...
      - (** lock release *)
        ...
        + exfalso.
          rewrite gLockSetRes gsoThreadRes in Hperm';
            now eauto.
      (* - (** thread spawn 
                in openmp sem we disable the spawn instruction *)
        destruct (tid == tidn) eqn:Heq; move/eqP:Heq=>Heq; subst.
        + pf_cleanup.
          eexists.
          right.
      ...
      - exfalso.
        ...
        + exfalso.
          rewrite gRemLockSetRes gsoThreadRes in Hperm';
            now eauto.
    (* FIXME permission is also decreased in openmp spawn *)
    Admitted.
    ...
    Lemma data_permission_decrease_execution:
      ...
      exists tr_pre tru U'' U''' tp_pre tre_pre m_pre tp_dec tre_dec m_dec,
        multi_step (U, tr, tpi, trei) mi (U'', tr ++ tr_pre, tp_pre, tre_pre) m_pre /\
        MachStep (U'', tr ++ tr_pre, tp_pre, tre_pre) m_pre
                 (U''', tr ++ tr_pre ++ tru, tp_dec, tre_dec) m_dec /\
        multi_step (U''', tr ++ tr_pre ++ tru, tp_dec, tre_dec) m_dec
                   (U', tr ++ tr', tpj, trej) mj /\
        (exists evu,
...
- Acquiring a lock *)
        ...
        + apply app_inv_head in H7; subst.
          ...
          { (** Case the lock is still there*)
            ...
            { (** Case permissions did not decrease*)
              ...
              (** And we can apply the IH*)
              (* destruct (IHU _ _ _ _ _ _ _ _ _ _ _ _ _ _ Hres'' Hres' H11 Hstable Hperm')
    ...
    Proof.
      ...
      generalize dependent tpi.
      generalize dependent tpj.
      generalize U, U', tr, tr', trei, trej, rmapi, rmapj.
      induction Hexec; intros; subst.
      - apply JMeq_eq in H0; inv H0.
        apply app_eq_refl_nil in H2; subst.
        rewrite Hres in Hres'; inv Hres';
      ...
      - apply JMeq_eq in H0, H1; inv H0; inv H1.
        ...
        { (** Case the lock is still there*)
          ...
          { (** Case permissions did not decrease*)
            rewrite app_assoc in IHHexec.
            (** And we can apply the IH*)
              destruct (IHHexec _ _ _ _ _ _ _ _ _ Hres' _ Hres'' JMeq_refl JMeq_refl Hstable Hperm')
              as (v & ev & Hnth & Hact & Hloc).
            exists ((length tr'0) + v)%nat, ev.
            split.
            rewrite <- nth_error_app; auto.
          ...
          { (** Case permissions decreased *)
            destruct (lockRes_lock_permission_decrease_step _ _ _ _ Hres Hres'' H Hperm Hdecr)
              as (ev & ? & Hact & Hloc); subst.
            exists O, ev.
            simpl;
              now auto.
        ...
        { (** Case the lock is removed *)
          exfalso.
          (* eapply lockRes_freelock_step in H; eauto.
          destruct H8 as (? & ? & ? & ? & Hempty).
          specialize (Hempty b ofs).
    ...
    Proof.
      ...
      inv Hstep;
        ...
        destruct U; inversion HschedN; subst; pf_cleanup;
          ...
          try (rewrite Hres in Hres'; inv Hres');
          try  (exfalso; by eauto);
          try apply app_inv_head in H6;
          try apply app_inv_head in H7; subst.
      - (** initial core step case *)
        exfalso.
        rewrite gsoThreadLPool in Hres'.
        rewrite Hres in Hres'; inv Hres';
    ...
    Fixpoint lockRes_data_permission_add_execution U
             U' tr tpi trei mi tpj trej mj tr' laddr rmapj b ofs
             (Hres: lockRes tpi laddr = None)
             (Hres': lockRes tpj laddr = Some rmapj)
             (Hexec: multi_step (U, tr, tpi, trei) mi (U', tr ++ tr', tpj, trej) mj)
    ...
    lockRes_data_permission_increase_execution U {struct U}:
      forall U' tr tpi trei mi tpj trej mj tr' laddr rmapi rmapj b ofs
        (Hres: lockRes tpi laddr = Some rmapi)
        (Hres': lockRes tpj laddr = Some rmapj)
        (Hexec: multi_step (U, tr, tpi, trei) mi (U', tr ++ tr', tpj, trej) mj)
        (Hperm: ~ Mem.perm_order'' (rmapi.1 !!!! b ofs) (Some Readable))
    ...
    Proof.
      { 
        ...
        generalize dependent tpi.
        generalize dependent tpj.
        generalize U, U', tr, tr', trei, trej, rmapj.
        dependent induction Hexec; intros; subst.
        - apply JMeq_eq in H0. inv H0.
          apply app_eq_refl_nil in H2; subst.
          pf_cleanup. by congruence.
        -  apply JMeq_eq in H0, H1. inv H0; inv H1.
            apply app_inv_head in H3; subst. clear Hexec.
            destruct (lockRes tp' laddr) as [rmap'|] eqn:Hres''.
    ...
    Proof.
      ...
      generalize_eqs Hexec.
        ...
        - apply JMeq_eq in H0, H1. inv H0; inv H1.
          ...
          * destruct (thread_spawn_step _ cnti cnt' H) as [ev [? ?]].
            subst.
            eexists [:: ], ev, _, _, _, _, _, _, _, _.
            rewrite !app_nil_r app_nil_l.
            repeat split; eauto. constructor.
          * erewrite! app_assoc in *.
            destruct (IHHexec _ _ _ _ _ _ _ cntj _ not_cnt'  JMeq_refl JMeq_refl )
              as (tr_pre & evu & U'' & U''' & tp_pre & tre_pre & m_pre & tp_inc & tre_inc & m_inc
                  & Hexec_pre & Hstep & Hexec_post & Hevu).
            eexists (tr'0 ++ tr_pre), evu, U'', U''', tp_pre, tre_pre, m_pre, tp_inc, tre_inc, m_inc.
            erewrite! app_assoc in *.
            repeat split; eauto.
            rewrite <- app_assoc.
            econstructor; eauto.
